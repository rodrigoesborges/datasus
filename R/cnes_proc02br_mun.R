#' Scrapes CNES's Health Human Resources/Occupations database at the city/region/state level
#'
#' This function allows the retrieval of data from CNES's
#' Health Occupations database much in the same way done at the online portal.
...
#' @usage
#' cnes_proc02br_mun(linha = "Município", coluna = "Não ativa", conteudo = 1,
...
#' @seealso \code{\link{cnes_prid02br_mun}}
#' @examples
#' \dontrun{
#' ## Requesting data from the city of Teresina/PI
#' cnes_proc02br_mun(municipio = 221100)
#' }
#'
#' @keywords CNES datasus health_occupations
#' @importFrom magrittr %>%
#' @importFrom utils head
#' @export

cnes_proc02br_mun <- function(linha = "Munic\u00edpio", coluna = "N\u00e3o ativa", conteudo = 1,
                              periodo = "last", regiao = "all", unidade_da_federacao = "all",
                              municipio = "all", municipio_gestor = "all", capital = "all",
                              cir = "all", macrorregiao_de_saude = "all", microrregiao_ibge = "all",
                              ride = "all", territorio_da_cidadania = "all", mesorregiao_pndr = "all",
  ...
  site <- httr::POST(url = "http://tabnet.datasus.gov.br/cgi/tabcgi.exe?cnes/cnv/proc02br.def",
                     body = form_data)
